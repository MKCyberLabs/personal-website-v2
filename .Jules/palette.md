...
## 2024-08-27 - Remove redundant aria-labels from dynamic navigation links
**Learning:** Applying a static `aria-label` (e.g., `aria-label="about"`) to a navigation link that contains dynamic or customizable visible text overrides the visible text for screen readers, causing confusion and poor accessibility.
**Action:** Only use `aria-label` when visible text is absent or insufficient. Do not apply it to links with dynamic text content.
